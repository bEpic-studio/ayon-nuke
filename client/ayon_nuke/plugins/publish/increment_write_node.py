...
def __process_writes_sync(publish_item, instance):
...
class IncrementWriteNodePathPreSubmit(
    ...
    label = "Increment path in Write node - Pre Submit"
    ...
    def process(self, instance):
        __process_writes_sync(self, instance)
...
class IncrementWriteNodePathPostSubmit(
    ...
    This allows users to manually trigger a local render being sure the render ouput paths are updated.
    """

    order = pyblish.api.IntegratorOrder + 10
    label = "Increment path in Write node - Post Workfile Increment"
    hosts = ["nuke", "nukeassist"]
    families = ["render", "prerender", "image"]
